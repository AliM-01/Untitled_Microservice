...
namespace ServiceHost.ApiClient.Infrastructure;
...
public class ApiBuilder
{
    ...
    public void IncludePath(string path)
    {
        ...
        {
            string newPath = $"{_builder.Path}/{path}";
    ...
    public ApiBuilder AddQueryString(string name, string value)
    {
        ...
        if (string.IsNullOrEmpty(qsNv[name]))
            qsNv[name] = string.Concat(qsNv[name], ",", value);
        else
            qsNv[name] = value;
